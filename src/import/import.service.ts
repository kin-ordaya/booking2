...
import { RecursoCursoModalidadService } from '@/recurso_curso_modalidad/recurso_curso_modalidad.service';
...
export class ImportService {
  ...
  constructor(
    ...
    private readonly recursoCursoModalidadService: RecursoCursoModalidadService,
  ...
  private async procesarRecursoCurso(row: any): Promise<any> {
    ...
    // const curso = await this.cursoService.findOneByCodigo(row.codigo_curso);
    // if (!curso) throw new NotFoundException('Curso no encontrado');
    const cursoModalidad = await this.cursoModalidadService.findOneByIDCursoAndModalidad(row.codigo_curso, row.modalidad);
    if (!cursoModalidad) throw new NotFoundException('CursoModalidad no encontrado');

    return await this.recursoCursoModalidadService.create({
      recurso_id: recurso.id,
      curso_modalidad_id: cursoModalidad.id,
    });
  }
